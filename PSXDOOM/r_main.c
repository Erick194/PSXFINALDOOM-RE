...
void R_RenderPlayerView(void)//L80030B80()
{
	RECT    area;
	angle_t angle;
...
inline int SlopeDiv(unsigned num, unsigned den)//L800310A4()
{
	unsigned ans;
	if (den < 512)
		return SLOPERANGE;
...
angle_t R_PointToAngle2(fixed_t x1, fixed_t y1, fixed_t x2, fixed_t y2)//L800310EC()
{
	int		x;
	int		y;
...
===============================================================================
*/
inline int	R_PointOnSide(int x, int y, node_t *node)//L80031400()
{
	fixed_t	dx, dy;
	fixed_t	left, right;
...
==============
*/
struct subsector_s *R_PointInSubsector(fixed_t x, fixed_t y)//L800314A8()
{
	node_t	*node;
	int		side, nodenum;
