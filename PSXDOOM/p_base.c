...
void P_RunMobjBase(void)//L80013D14()
{
	checkcount = 0;
...
void P_XYMovement(mobj_t *mo)//L80013DAC()
{
	fixed_t xleft, yleft;
	fixed_t xuse, yuse;
...
*/
//inline
void P_FloatChange(mobj_t *mo)//L8001400C()
{
	mobj_t *target;
	fixed_t dist, delta;
	fixed_t dx, dy;
...
void P_ZMovement(mobj_t *mo)//L800140D4()
{
...
void P_MobjThinker(mobj_t *mobj)//L800142B4()
{
	state_t *st;
	statenum_t state;
...
boolean PB_TryMove(int tryx, int tryy)//L800143D4()
{
	testx = tryx;
	testy = tryy;
...
void PB_UnsetThingPosition(mobj_t *thing)//L800144B4()
{
	int blockx, blocky;
...
void PB_SetThingPosition(mobj_t *thing)//L800145B0()
{
	sector_t     *sec;
	int           blockx, blocky;
	mobj_t      **link;
...
boolean PB_CheckPosition(void)//L800146B0()
{
...
//inline
boolean PB_BoxCrossLine(line_t *ld)//L80014894()
{
	fixed_t x1, x2;
	fixed_t lx, ly;
	fixed_t ldx, ldy;
...
//inline
boolean PB_CheckLine(line_t *ld)//L800149AC()
{
...
boolean PB_CheckThing(mobj_t *thing)//L80014A98()
{
	fixed_t  blockdist;
	int      delta;
	mobj_t  *mo;
...
boolean PB_BlockLinesIterator(int x, int y) //L80014BC4()
{
	int     offset;
	short  *list;
	line_t *ld;
...
boolean PB_BlockThingsIterator(int x, int y)//L80014E8C()
{
	mobj_t *mobj;
