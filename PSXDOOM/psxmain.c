...
void I_Error(char *error, ...)//L80032DE0()
{
    int Fnt;
    char buffer[256];
	va_list args;
...
void ImageToVram(psxobj_t *pic, char *name, int lump)//L80032EF8()
{
	int w, h;
	byte *data;
...
void DrawStaticImage(psxobj_t *psxobj, int xpos, int ypos, int pal)//L80033008()
{
	TextureCache(psxobj);
	DrawImage(psxobj->vtpage, pal, xpos, ypos, psxobj->vramx, psxobj->vramy, psxobj->w, psxobj->h);
}

void DrawImage(int vtpage, int pal, int xpos, int ypos,  char vramx, char vramy, int width, int height)//L80033088()
{
...
void P_LoadingPlaque(psxobj_t *psxobj, int xpos, int ypos, int pal)//L800335E4()
{
	DrawSync(0);
...
void NextTextureCacheIdx(void)//L800336DC()
{
    TextureCacheIdx += 1;
}
...
void DrawRender(void)//L800336F4()
{
	DrawSync(0);
...
void Init_Vram_Cache(void)//L80033830()
{
...
void TextureCache(psxobj_t *psxobj)//L8003387C()
{
	RECT			frame;
	psxobj_t	    **imgobj;
...
void V_ClearBlock(psxobj_t *psxobj)//L80033C10()
{
	unsigned long   *vmptr;
	int             x, y;
	short		    xs, ys;
...
void Valloc_Init(void)//L80033C80()
{
	int			    obj, j, i;
	short		    xs, ys;
	psxobj_t	    *psxobj;
...
void Vram_Viewer(int page)//L80033DC8()
{
    psxobj_t    **imgobj;
	psxobj_t    *psxobj;
...
void Link_Conection(void)//L80034A30()
{
...
boolean Update_Conection(void)//L80034EEC()
{
...
void Sync_Read_Write (void) //L800351A0()
{
	int sync;
...
void UpdateDrawOTag(void)//L800352E4()
{
...
unsigned int Get_CfgCode(unsigned int *cfgdata)//L80035330()
{
    unsigned int *tmpcfg;
