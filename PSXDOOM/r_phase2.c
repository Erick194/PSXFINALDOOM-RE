...
void R_RenderSKY(void)//L8002C5B8
{
	RECT    rect;
...
void R_RenderAll(subsector_t *sub)//L8002CC34()
{
	SVECTOR		 v0;
	VECTOR		 v1;
	long		 flag[4];
...
void R_FrontZClip(vleaf_t *vlf_in, vleaf_t **vlf_out)//L8002cfc8()
{
	vertex_t    *vrt;
...
int R_EdgeClipSide(int mode, vleaf_t **vlf)//L8002D2A4()
{
	leaf_t	*lf;
...
int R_LeftEdgeClip(vleaf_t *vlf_in, vleaf_t **vlf_out)//L8002D3A4()
{
    vertex_t    *vrt;
	leaf_t      *lfin, *lfout, *lfnext;
	int num_leafs, leafs_cnt, i;
...
int R_RightEdgeClip(vleaf_t *vlf_in, vleaf_t **vlf_out)//L8002D648()
{
    vertex_t    *vrt;
	leaf_t      * lfin, *lfout, *lfnext;
	int num_leafs, leafs_cnt, i;
...
void R_WallPrep(leaf_t *lf)//L8002D8E8()
{
	psxobj_t *texture;
...
void R_PlanePrep(vleaf_t *vlf1, PlaneType planeType)//L8002E7F4()
{
	RECT rect;
	int pic;
	fixed_t zheight;
...
void R_Render_Sprite(subsector_t *sub)//L8002F87C()
{
	mobj_t  *thing;
	fixed_t tx, tz;
	spritedef_t		*sprdef;
...
void R_Render_Hud_Weapons(void)//L80030380()
{
	int				i;
	pspdef_t		*psp;
	state_t			*state;
