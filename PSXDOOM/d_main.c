...
void D_DoomMain (void) //L8001267C()
{
	byte		*data;
...
int RunDemo(char *demoname)//L8001281C()
{
	int pfile;
	int exit;
...
void D_DebugSetPrintPos(int x, int y)//L800128E0()
{
	debugX = x;
	debugY = y;
}

void D_DebugPrint(const char *text, ...)//L800128F0()
{
	char buffer[256];
	va_list args;
...
void D_memset(void *dest, int val, int count)//L80012C48()
{
	byte	*p;
	int		*lp;
...
void D_memcpy(void *dest, void *src, int count)//L80012D04()
{
	byte	*d, *s;
...
void D_strncpy(char *dest, char *src, int maxcount)//L80012D38()
{
	byte	*p1, *p2;
	p1 = (byte *)dest;
	p2 = (byte *)src;
...
int D_strncasecmp(char *s1, char *s2, int len)//L80012D74()
{
	while (*s1 && *s2)
	{
		if (*s1 != *s2)
...
void D_strupr(char *s)//L80012DCC()
{
	char	c;
...
int	rndindex = 0;   //||uGp00000028
int prndindex = 0;  //||uGp0000002c

int P_Random(void)//L80012E10()
{
	prndindex = (prndindex + 1) & 0xff;
	return rndtable[prndindex];
}

int M_Random(void)//L80012E3C()
{
	rndindex = (rndindex + 1) & 0xff;
	return rndtable[rndindex];
}

void M_ClearRandom(void) // L80012E68()
{
	rndindex = prndindex = 0;
}

void M_ClearBox(fixed_t *box)//L80012E78()
{
	box[BOXTOP] = box[BOXRIGHT] = MININT;
	box[BOXBOTTOM] = box[BOXLEFT] = MAXINT;
}

void M_AddToBox(fixed_t *box, fixed_t x, fixed_t y)//L80012E98()
{
	if (x<box[BOXLEFT])
		box[BOXLEFT] = x;
...
void M_AddToBox2(fixed_t *box, fixed_t x, fixed_t y)//L80012F08()
{
	if (x<box[BOXLEFT])
		box[BOXLEFT] = x;
	if (x>box[BOXRIGHT])
...
int MiniLoop(void(*start)(void), void(*stop)(int), int(*ticker)(void), void(*drawer)(void))//L80012F70()
{
	int		exit;
	int		buttons;
