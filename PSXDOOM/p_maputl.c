...
fixed_t P_AproxDistance (fixed_t dx, fixed_t dy)//L8001C4A0()
{
	dx = abs(dx);
	dy = abs(dy);
	if (dx < dy)
...
int P_PointOnLineSide (fixed_t x, fixed_t y, line_t *line)//L8001C4D8()
{
	fixed_t	dx,dy;
	fixed_t	left, right;
...
void P_LineOpening (line_t *linedef)//L8001C6CC()
{
	sector_t	*front, *back;
...
void P_SetThingPosition (mobj_t *thing)//L8001C878()
{
	subsector_t  *ss;
    sector_t     *sec;
    int           blockx, blocky;
...
boolean P_BlockLinesIterator (int x, int y, boolean(*func)(line_t*) )//L8001C9B0()
{
	int			offset;
	short		*list;
	line_t		*ld;
...
boolean P_BlockThingsIterator (int x, int y, boolean(*func)(mobj_t*) )//L8001CAD0()
{
	mobj_t		*mobj;
