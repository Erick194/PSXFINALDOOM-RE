...
/* */
/*================================================== */
void ST_InitEveryLevel(void)//L80039178()
{
	int		i;
...
void ST_DrawValue(int x, int y, int value)//L8003AF30()
{
	int index, i;
...
void ST_DrawMessage(int x, int y, char *text)//L8003B53C()
{
	unsigned int c,cc;
...
void ST_CheatDraw(void)//L8003B86C()
{
	char		text[64];
	player_t	*p;
...
void ST_doPaletteStuff(void)//L8003BC58()
{
	int		palette_;
	int		cnt;
	int		bzc;
...
int ST_GetTextCenterX(char *text)//L8003BDA0()
{
	int c;
	int width;
...
void ST_DrawText(int x, int y, char *text)//L8003BE8C()
{
	int		xpos;
	int		ypos;
	int		c;
