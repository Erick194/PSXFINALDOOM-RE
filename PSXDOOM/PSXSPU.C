...
void psxspu_init_reverb(long  rev_mode, short depthleft, short depthright, long  delay, long  feedback)//L80045f70()
{
    psxspu_status = 0;
...
void psxspu_set_reverb_depth(short depthleft,short depthright)//L80046050()
{
    psxspu_status = 0;
    rev_attr.depth.left = depthleft;
	rev_attr.depth.right = depthright;
...
void psxspu_init(void)//L80046098()
{
...
void psxspu_set_master_volume(int vol)//L80046144()
{
    SpuCommonAttr com_attr;
...
void psxspu_set_cd_volume(int vol)//L80046188()
{
    SpuCommonAttr com_attr;
...
void psxspu_setcdmixon(void)//L800461CC()
{
    SpuCommonAttr com_attr;
...
void psxspu_setcdmixoff(void)//L80046214()
{
    SpuCommonAttr com_attr;
...
void psxspu_fadeengine(void)//L80046254()
{
    if (psxspu_status != 0)
	{
...
void psxspu_set_cd_vol(int vol)//L80046368()
{
    psxspu_status = 0;
    psxspu_cd_vol = vol;
...
int  psxspu_get_cd_vol(void)//L800463C4()
{
    return psxspu_cd_vol;
}

void psxspu_start_cd_fade(int msec, int destvol)//L800463D4()
{
    psxspu_status = 0;
...
void psxspu_stop_cd_fade(void)//L8004648C()
{
    psxspu_status = 0;
	psxspu_cd_fadecount = 0;
	psxspu_status = 1;
...
int  psxspu_get_cd_fade_status(void)//L800464B0()
{
    return (psxspu_cd_fadecount < 2) ^ 1;
}

void psxspu_set_master_vol(int vol)//L800464C8()
{
    psxspu_status = 0;
...
int  psxspu_get_master_vol(void)//L80046524()
{
    return psxspu_master_vol;
}

void psxspu_start_master_fade(int msec, int destvol)//L80046534()
{
    psxspu_status = 0;
	if (WessTimerActive == 0)
    {
...
void psxspu_stop_master_fade(void)//L800465EC()
{
    psxspu_status = 0;
	psxspu_master_fadecount = 0;
	psxspu_status = 1;
...
int  psxspu_get_master_fade_status(void)//L80046610()
{
